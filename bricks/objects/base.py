"""Bricks common internal object model"""
...
from bricks.objects import utils as obj_utils
...
from bricks.openstack.common.rpc import serializer as rpc_serializer
...
def make_class_properties(cls):
    cls.fields.update(BricksObject.fields)
...
class BricksObjectMetaclass(type):
    ...
    def __init__(cls, names, bases, dict_):
        if not hasattr(cls, '_obj_classes'):
            # This will be set in the 'BricksObject' class.
            cls._obj_classes = collections.defaultdict(list)
        else:
            # Add the subclass to BricksObject._obj_classes
...
def remotable_classmethod(fn):
    ...
    def wrapper(cls, context, *args, **kwargs):
        if BricksObject.indirection_api:
            result = BricksObject.indirection_api.object_class_action(
                context, cls.obj_name(), fn.__name__, cls.version,
                args, kwargs)
        else:
            result = fn(cls, context, *args, **kwargs)
            if isinstance(result, BricksObject):
...
def remotable(fn):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if BricksObject.indirection_api:
            updates, result = BricksObject.indirection_api.object_action(
...
@six.add_metaclass(BricksObjectMetaclass)
class BricksObject(object):
    ...
    #          }
    #
    # NOTE(danms): The base BricksObject class' fields will be inherited
    ...
    def obj_from_primitive(cls, primitive, context=None):
        ...
        """
        if primitive['bricks_object.namespace'] != 'bricks':
            # NOTE(danms): We don't do anything with this now, but it's
            # there for "the future"
            raise exception.UnsupportedObjectError(
                objtype='%s.%s' % (primitive['bricks_object.namespace'],
                                   primitive['bricks_object.name']))
        objname = primitive['bricks_object.name']
        objver = primitive['bricks_object.version']
        objdata = primitive['bricks_object.data']
        ...
        changes = primitive.get('bricks_object.changes', [])
    ...
    def obj_to_primitive(self):
        ...
        obj = {'bricks_object.name': self.obj_name(),
               'bricks_object.namespace': 'bricks',
               'bricks_object.version': self.version,
               'bricks_object.data': primitive}
        if self.obj_what_changed():
            obj['bricks_object.changes'] = list(self.obj_what_changed())
        return obj
...
class ObjectListBase(object):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            ...
            # NOTE(danms): We must be mixed in with an BricksObject!
    ...
    def _attr_objects_from_primitive(self, value):
        ...
        objects = []
        for entity in value:
            obj = BricksObject.obj_from_primitive(entity,
                                                  context=self._context)
            objects.append(obj)
        return objects


class BricksObjectSerializer(rpc_serializer.Serializer):
    """A BricksObject-aware Serializer.

    This implements the Oslo Serializer interface and provides the
    ability to serialize and deserialize BricksObject entities. Any service
    that needs to accept or return BrickObjects as arguments or result values
    ...
    def deserialize_entity(self, context, entity):
        if isinstance(entity, dict) and 'bricks_object.name' in entity:
            entity = BricksObject.obj_from_primitive(entity, context=context)
...
def obj_to_primitive(obj):
    ...
    An BricksObject becomes a dict, and anything that implements ObjectListBase
    becomes a list.
    """
    if isinstance(obj, ObjectListBase):
        return [obj_to_primitive(x) for x in obj]
    elif isinstance(obj, BricksObject):
        result = {}
